...
describe "biz logic revenue" do
  ...
  it "revenue across all merchants on a specific date" do
    ...
    result = Merchant.revenue({'date' => '2012-03-16T14:54:05.000Z'})
    expect(result).to eq({:data=>{:type=>"query_result", :attributes=>{:total_revenue=>"60.0"}}})
  end

  it "highest grossing merchant(s) w/ limit quantity" do
    result = Merchant.most_revenue({'quantity' => 2})

    expect(result[0].id).to eq(@merchant_2.id)
  end

  it "total revenue for merchant" do
    result = @merchant_1.revenue

    expect(result).to eq({:data=>{:type=>"query_result", :attributes=>{:revenue=>"40.0"}}})
  end
